#!/usr/bin/env python3
"""
Anchor-free (centered) opposite-spin mask diagnostic for H10 R=1.6.

Reuses the validated pipeline but replaces opp_spin_sites with a
centroid-based selection that is invariant to global translation.
"""
...
OUT = Path("outputs/h10_anchor_free")
OUT.mkdir(parents=True, exist_ok=True)
CSV = OUT / "results.csv"

SHOTS = int(os.environ.get("ANCHOR_FREE_SHOTS", p.CFG["shots"]))
SEEDS = tuple(p.CFG["seeds"])
N_DETS = p.CFG["n_dets"]
K_OS = 4  # number of opposite-spin on-site terms to retain


def centered_opp_spin_sites(pos, centroids, k=K_OS):
    """
    Select the k orbitals whose positions are closest to the centroid
    of all active orbitals. This is translation-invariant.
    """
    center = float(np.mean(centroids))
    dists = [(abs(float(centroids[int(orb)]) - center), int(orb))
             for orb in range(len(pos))]
    dists.sort()
    return [orb for _, orb in dists[:k]]


def interaction_pairs_centered(pos, centroids):
    aa = sorted({
        tuple(sorted(pair))
        for pair in p.same_spin_pairs(pos)
    })
    ab = sorted((orb, orb) for orb in centered_opp_spin_sites(pos, centroids))
    return aa, ab


def main():
    print("ANCHOR-FREE (CENTERED) MASK DIAGNOSTIC")
    print(f"H10 R={p.CFG['h10_R']} Å | shots={SHOTS} | seeds={SEEDS} | k_os={K_OS}")
    ...
    orderings = {
        "physical": physical,
        "physical_offset0": physical,
        "physical_offset1": physical,
    }
    ...
    # We only compare two effective masks here:
    # - the original best anchor (offset 0)
    # - the new centered mask
    # To keep runtime short, we run only physical ordering.

    for name, perm in [("physical", physical)]:
        pos = p.positions_from(perm)

        # Original best anchor (offset 0) for reference
        pairs_orig = p.interaction_pairs_for(pos)
        op_orig = p.build_ucj(
            t2_loc,
            t1_loc,
            interaction_pairs=pairs_orig,
        )

        # Centered mask
        pairs_c = interaction_pairs_centered(pos, centroids)
        op_c = p.build_ucj(
            t2_loc,
            t1_loc,
            interaction_pairs=pairs_c,
        )

        retained = p.retained_J_of(pos, Jaa, Jab)

        for mask_name, op in [("anchor0", op_orig), ("centered", op_c)]:
            for seed in SEEDS:
                ...
                afile = OUT / f"_a_{name}_{mask_name}_{seed}.txt"
                bfile = OUT / f"_b_{name}_{mask_name}_{seed}.txt"
                ...
                print(
                    f"{name:12s} mask={mask_name:8s} seed={seed} "
    ...
    summary = (
        ok.groupby(["mask"], as_index=False)
        .agg(
