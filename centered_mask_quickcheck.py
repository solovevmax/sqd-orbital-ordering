...
OUT = Path("outputs/h10_centered_quick")
OUT.mkdir(parents=True, exist_ok=True)
CSV = OUT / "results.csv"

SHOTS = int(os.environ.get("CENTERED_SHOTS", p.CFG["shots"]))
SEEDS = tuple(p.CFG["seeds"])
N_DETS = p.CFG["n_dets"]

def main():
    print("CENTERED-MASK QUICK CHECK (3 orderings)")
    ...
    C_loc, Uo, Uv = p.block_boys(mol, mf, active, nocc, tag="cq-")
