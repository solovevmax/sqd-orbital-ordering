...
OUT = Path("outputs/h10_end_weighted_quick")
OUT.mkdir(parents=True, exist_ok=True)
CSV = OUT / "results.csv"

SHOTS = int(os.environ.get("END_WEIGHTED_SHOTS", p.CFG["shots"]))
SEEDS = tuple(p.CFG["seeds"])
N_DETS = p.CFG["n_dets"]

# Override mask mode for this experiment
p.CFG["mask_mode"] = "end_weighted"

def main():
    print("END-WEIGHTED MASK QUICK CHECK (3 orderings)")
    ...
    C_loc, Uo, Uv = p.block_boys(mol, mf, active, nocc, tag="ew-")
    ...
    Jaa, Jab = p.diag_coulomb(p.build_ucj(t2_loc, t1_loc))
    ...
    for name, perm in orderings.items():
        ...
        pairs = p.interaction_pairs_for(pos, centroids)
